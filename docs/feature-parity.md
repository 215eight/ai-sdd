...
| 4 | **Create agent (role) per stack** | One Worker per role + Traits for the stack; `factory worker init/fork` scaffolding | ✅ | Covered by concept #2 (ADR-0021 / 0022 / 0024). |
...
- `factory run <feature>` — run one cycle (one Run). *(#5)*
- `factory run --batch [--serial]` — run multiple cycles serially. *(#6; parallel is #7, optional)*
- `factory worker init | fork` — scaffold role Workers. *(#4)*
- `factory bootstrap <repo>` — generate a stack's conventions (Trait) + verification (Checks). *(#3)*
- `factory validate` — validate all specs (structural + schema + referential). *(ADR-0020)*

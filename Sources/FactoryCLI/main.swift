...
struct Graph: ParsableCommand {
    ...
    private func projectDoc() throws -> String {
        ...
        let factoryURL = URL(fileURLWithPath: dir, isDirectory: true)
        if let env = try? loader.loadPipeline(atDirectory: factoryURL) {
        ...
        let featuresDir = factoryURL.appendingPathComponent("features", isDirectory: true)
        ...
        for entry in entries {
            ...
            if let env = try? loader.loadPipeline(atDirectory: entry) {
                ...
                sections.append(.init(heading: "Feature · \(name)",
                                      body: "> ⚠ could not load `\(name)/pipeline.yaml` — see `factory validate \(entry.path)`."))
            }
        }

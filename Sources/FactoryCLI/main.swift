...
struct Graph: ParsableCommand {
    ...
    @Argument(help: "Directory containing pipeline.yaml (a feature graph, or the build pattern).")
    var dir: String
    @Option(name: .long, help: "Write the Markdown here instead of stdout.")
    var out: String?
    ...
    func run() throws {
        let (env, _, _) = try loadValidated(dir)
        let doc = "# \(env.metadata.name) — dependency graph\n\n"
            + "_Generated by `factory graph` from the pipeline spec (ADR-0027). Renders in any Markdown viewer._\n\n"
            + GraphRenderer.mermaid(env.spec, direction: direction) + "\n"
        if let out {
            try doc.write(toFile: out, atomically: true, encoding: .utf8)
            print("✓ wrote \(out)  ·  \(env.spec.nodes.count) nodes, \(env.spec.edges.count) edges")
        } else {
            print(doc)
        }
    }
